...
CONTAINER_INDEX_URL = reverse('horizon:sdscontroller:containers:index')
...
class SwiftTests(test.TestCase):
    ...
    def test_create_container_get(self):
        res = self.client.get(reverse('horizon:sdscontroller:containers:create'))
        self.assertTemplateUsed(res, 'sdscontroller/containers/create.html')
        ...
        for container in self.containers.list():
            for obj in self.objects.list():
                ...
                download_url = reverse(
                    'horizon:sdscontroller:containers:object_download',
    ...
    def test_copy_index(self):
        ...
        res = self.client.get(reverse('horizon:sdscontroller:containers:object_copy',
                                      args=[self.containers.first().name,
                                            self.objects.first().name]))
        self.assertTemplateUsed(res, 'sdscontroller/containers/copy.html')
        self.assertNotContains(res, INVALID_CONTAINER_NAME_1)
        self.assertNotContains(res, INVALID_CONTAINER_NAME_2)
    ...
    def test_view_container(self):
        for container in self.containers.list():
            ...
            view_url = reverse('horizon:sdscontroller:containers:container_detail',
                               args=[container.name])
            res = self.client.get(view_url)

            self.assertTemplateUsed(res,
                                    'sdscontroller/containers/container_detail.html')
        ...
        for container in self.containers.list():
            for obj in self.objects.list():
                ...
                api.swift.swift_get_object(IsA(http.HttpRequest),
                    ...
                    .AndReturn(obj)
                self.mox.ReplayAll()
                view_url = reverse('horizon:sdscontroller:containers:object_detail',
                                   args=[container.name, obj.name])
                res = self.client.get(view_url)

                self.assertTemplateUsed(
                    res, 'sdscontroller/containers/object_detail.html')
